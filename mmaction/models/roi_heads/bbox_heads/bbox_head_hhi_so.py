...
class BBoxHeadHHI_SO(nn.Module):
    def __init__(
            self,
        ...
        super(BBoxHeadHHI_SO, self).__init__()
        ...
        if self.use_spatial:
            cls_in_channels = 1280
        else:
            cls_in_channels = 1024
    ...
    def forward(self, rois, x, p1_inds, p2_inds, x_union) -> Tensor:
        ...
        if self.use_spatial:
            ...
            x_in = torch.cat([p1_rep, p2_rep, spatial_enc], 1)
        else:
            # x_in = torch.cat([p1_rep, p2_rep, vr_rep], 1)
            x_in = torch.cat([p1_rep, p2_rep], 1)
...
if mmdet_imported:
    MMDET_MODELS.register_module()(BBoxHeadHHI_SO)