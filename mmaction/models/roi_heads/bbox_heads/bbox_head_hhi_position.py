...
class BBoxHeadHHIPosition(nn.Module):
    def __init__(
            self,
        ...
        super(BBoxHeadHHIPosition, self).__init__()
    ...
    def forward(self, rois, x, p1_inds, p2_inds, x_union) -> Tensor:
        ...
        if self.use_spatial:
            ...
            union_boxes = torch.cat((p1_boxes, p2_boxes), dim=1)
            assert union_boxes.shape[1] == 8
            assert union_boxes.shape[0] == p1_boxes.shape[0]
            # unions = boxes2union(p1_boxes, p2_boxes, 27)
            # spatial_enc = self.conv(unions)
            # spatial_enc = self.sp_fc(spatial_enc.view(-1, 256*7*7))
            spatial_enc = self.sp_fc(union_boxes)
...
if mmdet_imported:
    MMDET_MODELS.register_module()(BBoxHeadHHIPosition)