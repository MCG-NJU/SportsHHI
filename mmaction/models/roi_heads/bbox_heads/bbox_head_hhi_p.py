...
class BBoxHeadHHI_P(nn.Module):
    def __init__(
        ...
        super(BBoxHeadHHI_P, self).__init__()
        assert temporal_pool_type in ['max', 'avg']
        assert spatial_pool_type in ['max', 'avg']
        
        if not use_spatial:
            print("Should always use spatial for ablation 'p'.")
            use_spatial = True
        ...
        self.vr_fc = nn.Linear(in_channels, 256)

        if self.use_spatial:
            cls_in_channels = 256
        else:
            raise ValueError("Should use spatial.")

        if self.use_attention:
            self.transformer = transformer(enc_layer_num=1, embed_dim=cls_in_channels, dropout=dropout_ratio)
...
if mmdet_imported:
    MMDET_MODELS.register_module()(BBoxHeadHHI_P)